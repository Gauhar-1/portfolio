...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const updatedLinks = await Links.findOneAndUpdate(
      {},
      { profilePhotoUrl: secureUrl },
      { new: true, upsert: true }
    );

    return NextResponse.json({ message: 'Image uploaded successfully', url: secureUrl, data: updatedLinks }, { status: 200 });
  } catch (error) {
