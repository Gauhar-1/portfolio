...
import { ExternalLink, Github, Globe, Target, Wifi, ChevronRight, Scan, Layers } from 'lucide-react';
...
const ProjectSpotlight = () => {
    ...
    // FIX 4: Stable DOM rendering. The <section> is always mounted.
    return (
        <section ref={sectionRef} id="projects" className="bg-[#050505] text-slate-200 overflow-hidden font-sans h-[100dvh] w-full flex flex-col relative border-y-8 border-[#050505] z-10">
            ...
            {isLoading ? (
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-[#050505] font-mono text-emerald-500 text-xl md:text-2xl font-black uppercase tracking-widest px-4 text-center">
                                ...
                                <div key={project._id} className="project-panel w-[100vw] h-full flex-shrink-0 bg-[#050505] flex flex-col lg:flex-row relative group overflow-hidden">
                                        ...
                                        <div className="mt-4 md:mt-6 pt-4 border-t-2 border-white/10 shrink-0">
                                            <div className="flex flex-wrap gap-2 mb-6">
                                                {project.technologies.slice(0, 5).map(tech => ( 
                                                    <span key={tech} className="border border-slate-700 px-2 py-1 md:px-3 md:py-1 font-mono text-[10px] md:text-xs font-bold text-slate-300 uppercase hover:border-emerald-500 hover:text-emerald-500 transition-all bg-black">
                                                        {tech}
                                                    </span>
                                                ))}
                                    ...
                                    <div className="w-full lg:w-[55%] h-[50%] lg:h-full relative overflow-hidden z-10 bg-black order-1 lg:order-2 group/img cursor-crosshair">
                                        <div className="absolute inset-0 w-[130%] h-full parallax-img origin-center">
                                            {project.imageUrl ? (
                                                <Image src={project.imageUrl} alt={project.title} fill className="object-cover object-center filter grayscale contrast-125 brightness-50 group-hover/img:grayscale-0 group-hover/img:brightness-100 transition-all duration-700 ease-in-out" />
                                            ) : (
