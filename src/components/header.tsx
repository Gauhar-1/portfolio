...
const Header = ({ initialLinks }: headerProps) => {
  const [mounted, setMounted] = useState(false); // STRICT HYDRATION GUARD
  ...
  // --- 1. MOUNT & CLOCK ---
  useEffect(() => {
    setMounted(true); // Component is now safely hydrated
  ...
  // --- 3. SYSTEM SCROLL SPY (INTERSECTION OBSERVER) ---
  useEffect(() => {
    // 1. Handle the header background blur on scroll
    const handleScroll = () => {
      setScrolled(window.scrollY > 50);
    };
    window.addEventListener('scroll', handleScroll);

    // 2. Handle the Active Section Highlighting
    const sectionIds = NAV_LINKS.map(link => link.href.substring(1));

    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          // Update the active section only when it comes into the viewport
          if (entry.isIntersecting) {
            setActiveSection(entry.target.id);
          }
        });
      },
      // This creates a trigger "window" near the top of the screen.
      // Adjust these percentages if it triggers too early or too late.
    ...
    // Cleanup listeners and observers when component unmounts
    return () => {
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="hidden lg:flex items-center gap-4 border-l border-white/10 pl-6 ml-2">
            ...
            <div className="flex items-center gap-2 text-[10px] font-mono text-slate-500">
              {/* Only render actual time if mounted, else render static fallback */}
